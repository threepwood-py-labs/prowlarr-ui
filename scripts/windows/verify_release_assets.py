...
from package_release import load_nuitka_config
...
def expected_asset_names(
    ...
    for target_arch in target_arches:
        names.append(f"{output_name}-{release_tag}-{target_arch}.exe")
        names.append(f"{output_name}-{target_arch}-{release_tag}.zip")
...
def verify_release_assets(
    ...
    expected_names = set(expected_asset_names(output_name, release_tag, target_arches))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        output_name = load_nuitka_config(repo_root).output_name
        verify_release_assets(release_dir, output_name, release_tag, target_arches)
